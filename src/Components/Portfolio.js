...
export default function Portfolio({ data }) {
  ...
  useEffect(() => {
    setProjects(
      filter ? data.filter((project) => project.category === filter) : data
    );
  }, [filter, data]);
    ...
    <section id="portfolio">
      <div className="row">
        <div className="twelve columns collapsed">
          <h1 className={`${i18n.language === "ar" && "arabic"}`}>
            {t("Samples of My Work")}
          </h1>
          {filters.length > 0 && (
            <div className="twelve columns main-col text-center text-underline">
              <div
                className={`${filtersLength(
                  filters.length + 1
                )} columns main-col`}
              >
                <p>
                  <a
                    className={`button buttonWidth ${!filter && "selected"}`}
                    onClick={() => setFilter()}
                  >
                    {t("All")}
                  </a>
              ...
              {filters.map((filt, i) => (
                ...
                >
                  <p>
                    <a
                      className={`button buttonWidth ${
                        filter === filt && "selected"
                      }`}
